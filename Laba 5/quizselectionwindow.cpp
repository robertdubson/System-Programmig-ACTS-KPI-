...
void QuizSelectionWindow::on_quizzesListWidget_itemDoubleClicked(QListWidgetItem *item)
{
    ...
    ratingForm = new CurrentRatingForm();

    ratingForm->setCurrentRound(currentRound);
    ...
    for(int i = 0; i<selectdQuiz.getQuestions().size(); i++){
        ...
        quesForm->setCurrentRoundForm(ratingForm);
    ...
    playerNameForm = new PlayerNameForm();

    playerNameForm->setCurrentRatingForm(ratingForm);

    playerNameForm->show();

    //QMessageBox msgBox;
    //msgBox.setText("Ваш рейтинг складає " + currentRound->getRating() );
    //msgBox.exec();
