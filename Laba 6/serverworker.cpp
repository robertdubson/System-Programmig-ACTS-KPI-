    ...
    , m_serverSocket(new QTcpSocket(this))
{
    ...
    // створюємо об'єкт бази даних на стеку та присвоюємо як поле екземпляру serverworker
    //InMemoryDataBase *newDb;

    //newDb = new InMemoryDataBase;

    //this->setDataBase(*newDb);
...
{
    QMessageBox box;
    box.setText("here we are sending json");
    box.exec();
    ...
    if (json.value("type")=="quizzes"){

        emit logMessage("Відправляємо тести до клієнта.");
...
InMemoryDataBase ServerWorker::getDataBase(){

    return Db;

}

void ServerWorker::setDataBase(InMemoryDataBase inDB){
