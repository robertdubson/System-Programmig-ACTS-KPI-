...
void Client::jsonReceived(const QJsonObject &docObj)
{
    ...
    } else if(typeVal.toString() == "quizzes"){
        //box.setText("БЕБРА");
        //box.exec();
        emit quizzesReceivedOnClient(docObj);
...
void Client::selectedQuizSent(const int &selectedQuizID){




}
void Client::selectedVariantSent(const int &selectedVarID){}

void Client::sendReqForQuizzes(const QString name){
    ...
    // Create the JSON we want to send
    QJsonObject message;
    message[QStringLiteral("type")] = QStringLiteral("quizRequest");
    message[QStringLiteral("text")] = name;
    // send the JSON using QDataStream
    clientStream << QJsonDocument(message).toJson();
